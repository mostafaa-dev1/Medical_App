...
import 'package:medical_system/features/book_appointment/data/services/book_appointment_data.dart';
...
class BookAppointmentCubit extends Cubit<BookAppointmentState> {
  ...
  Future<void> getAvailableTimes(String date) async {
    appointmentTimes = [];
    emit(GetAvailableTimesLoading());
    print(date);
    final response =
        await _data.getAppointmentTimes(doctorId: 'DOC-253190009', date: date);
    response.fold((l) {
  ...
  Future<void> calculateTime(
      DateTime startTime, DateTime endTime, int duration) async {
  ...
  void selectedDateTime(
      Map<String, dynamic> date, Map<String, dynamic> time, String doctorId) {
    print(date);
    print(time);
    print(doctorId);
  ...
  Future<void> rescheduleAppointment(String id, String reason) async {
    print('ffff');
    print(id);
    print(reason);
